...
const Homepage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const fetchedPeople = response!.data.results.map((user: Person, index: number) => ({
          id: user.id.value,
          email: user.email,
  ...
  if(loading)
    return <div className="flex justify-center items-center min-h-screen bg-[#EA7AF4]">Loading...</div>
  if (error)
    return <div className="flex justify-center items-center min-h-screen bg-[#EA7AF4]">{error}</div>
  return (
    <div className="relative min-h-screen bg-gradient-to-b from-[#EA7AF4] to-[#8A2BE2] flex flex-col items-center gap-4">
      <div className="relative h-3/4 py-16 flex items-start justify-center gap-2 min-h-3.5 w-4/6">
        {people.map((person) => (
          <CircularImage key={person.email} src={person.src} size={person.size} style={person.style} />
        ))}
      </div>
      <div className="relative text-center text-white md:p-4">
        <h1 className="text-2xl font-bold md:text-3xl">The Best Social App To Make New Friends!</h1>
        <p className="md:text-2xl">Find people with the same interests as you</p>
      </div>
      <div className="flex flex-col gap-6">
        <HomeButton color={`text-[#290628]`} border={`border`} text="Sign Up" bgColor={`bg-white`}/>
        <LoginButton />
      </div>
    </div>
  );
